...
export default function CreateItem() {
    ...
    const [formInput, updateFormInput] = useState({receiver: '', name: '', description:'',price:''})
    ...
    async function createItem(){
        const {name, description, receiver, price} = formInput; //get the value from the form input
        
        //form validation
        if(!name || !description || !receiver || !fileUrl || !price) {
            return
        }

        const data = JSON.stringify({
            name, description, image: fileUrl, receiver
        });

        try{
            const added = await client.add(data)
            const url = `https://ipfs.infura.io/ipfs/${added.path}`
            //pass the url to save it on Polygon adter it has been uploaded to IPFS
            createSale(url)
        }
        catch(error){
    ...
    async function createSale(url){
        ...
        let contract = new ethers.Contract(tokenAddress, Tokenabi.abi, signer);
        ...
        contract = new ethers.Contract(tokenPlatformAddress, Platformabi.abi, signer);
        ...
        transaction = await contract.createCertificate(
            tokenAddress,formInput.receiver, tokenId, price, {value: listingPrice }
        )
